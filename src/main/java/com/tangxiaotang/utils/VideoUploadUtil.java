...
public class VideoUploadUtil {
	...
	// 账号AK信息请填写(必选)
	private static final String accessKeySecret = AliyunConfig.accessKeySecret;
	private static final String regionId = AliyunConfig.regionId;
	 ...
	 * @throws Exception 
     */
    public static Map<String,Object> uploadVideoByStream(String title, String fileName, InputStream inputStream) throws Exception {
        ...
        if (response.isSuccess()) {
            ...
            System.out.println("UploadAddress="+response.getMessage());
            map.put("VideoId", response.getVideoId());
            //获取源文件信息
            GetMezzanineInfoResponse  response1 = getMezzanineInfo(initVodClient(),response.getVideoId());
            //size是对的
            System.out.print("Mezzanine.Size = " + response1.getMezzanine().getSize() + "\n");
            System.out.print("Mezzanine.duration = " + response1.getMezzanine().getDuration() + "\n");
            //获取的视频时长为0?
            map.put("duration", response1.getMezzanine().getDuration());
        } else { 
